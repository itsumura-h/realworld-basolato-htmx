...
proc invoke*(self:TagFeedArticleListPresenter, tagName:string, page:int, isLogin:bool, loginUserId:string):Future[ArticlePreviewViewModel] {.async.} =
  ...
  let paginatorViewModel = PaginatorViewModel.new(paginatorDto, &"/island/feed/tag-feed/${tagName}", &"/tag-feed/{tagName}")

  var feedNavbarViewModelList = @[
    FeedNavbarViewModel.new(
      title = "Global Feed",
      isActive = false,
      hxGetUrl = "/island/feed/global-feed",
      hxPushUrl = "/"
    ),
    FeedNavbarViewModel.new(
      title = tagName,
      isActive = true,
      hxGetUrl = &"/island/feed/tag-feed/{tagName}",
      hxPushUrl = "/"
    ),
  ]
